...
class Asignacion {
  ...
  static async resyncAll(umbral = 3) {
    const { rows } = await query('SELECT ventas.fn_sync_todos($1) AS total', [umbral]);
    return rows[0]; // { total }
  }
}
